erpnext.PointOfSale.ItemCart = class {
    ...
    bind_events() {
        ...
        // customizations
        this.$cart_items_wrapper.on("change", ".sugar-options", function () {
            $.each($('.sugar-options:checked'), (k, v) => {
                // debugger;
                if ($(this).is(':checked')) {
                    // Get the data attributes for the checked radio
                    let sugar_value = $(v).val()
                    let rowId = $(v).data('rowid')
                    let fieldname = $(v).data('custom-fieldname')
                    let fieldnames = ["custom__add_more_sugar", "custom_less_sugar", "custom_diet"]
                    frappe.model.set_value("POS Invoice Item", rowId, fieldname, 1)
                    // Check if the fieldname is in the fieldnames array
                    let fieldIndex = $.inArray(fieldname, fieldnames);

                    // If the fieldname exists in the array, remove it
                    if (fieldIndex !== -1) {
                        fieldnames.splice(fieldIndex, 1); // Remove the fieldname from the array
                        // console.log(`Removed fieldname: ${ fieldname }`);
                    }
                    $.each(fieldnames, (k, v) => {
                        frappe.model.set_value("POS Invoice Item", rowId, v, 0)
                    })

                    // Output the values
                    console.log("Row ID: " + rowId);
                    console.log("Row value: " + sugar_value)
    ...
    render_cart_item(item_data, $item_to_update) {
        const currency = this.events.get_frm().doc.currency;
        const me = this;
        this.$sugar_options = `<!-- Add the checkboxes here -->
            <div style="display: flex; flex-direction: row; justify-content: flex-start; gap: 20px; width: 100%;padding-left: 10px;">
                <div style="display: flex;gap: 5px;flex-direction: row;align-items: stretch;">
                    <input type="radio" class="sugar-options" id="radio1-${escape(item_data.name)}" name="sugar-option-${escape(item_data.name)}" data-rowid="${escape(item_data.name)}" data-custom-fieldname="custom__add_more_sugar" value="Add More Sugar" ${item_data.custom__add_more_sugar ? "checked" : ""} style="margin-right: 0px !important;">
                    <label for="radio1-${escape(item_data.name)}" style="line-height: 1;">Add More Sugar</label>
                </div>

                <div style="display: flex;gap: 5px;flex-direction: row;align-items: stretch;">
                    <input type="radio" class="sugar-options" id="radio2-${escape(item_data.name)}" name="sugar-option-${escape(item_data.name)}" data-rowid="${escape(item_data.name)}" data-custom-fieldname="custom_less_sugar" value="Less Sugar" ${item_data.custom_less_sugar ? "checked" : ""} style="margin-right: 0px !important;">
                    <label for="radio2-${escape(item_data.name)}" style="line-height: 1;">Less Sugar</label>
                </div>

                <div style="display: flex;gap: 5px;flex-direction: row;align-items: stretch;">
                    <input type="radio" class="sugar-options" id="radio3-${escape(item_data.name)}" name="sugar-option-${escape(item_data.name)}" data-rowid="${escape(item_data.name)}" data-custom-fieldname="custom_diet" value="Diet" ${item_data.custom_diet ? "checked" : ""} style="margin-right: 0px !important;">
                    <label for="radio3-${escape(item_data.name)}" style="line-height: 1;">Diet</label>
                </div>
            </div>`
